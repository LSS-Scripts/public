// ==UserScript==
// @name         LSS - Summenleiste mit Warnfunktion (Stabile Version)
...
(function() {
    ...
    // Ein großer try-catch-Block fängt alle eventuellen Fehler im Skript ab
    // und verhindert so, dass die gesamte Seite nicht mehr lädt.
    try {
        // ============== KONFIGURATION via B&M Scriptmanager ==============
        const SCRIPT_NAME = 'LSS - Summenleiste mit Warnfunktion (Stabile Version)';
        ...
        if (!sumDisplayContainer) {
            ...
            // Stile direkt in die spans, um Abhängigkeiten zu reduzieren
            sumDisplayContainer.innerHTML = `
                <span id="mission-count-display" title="Sichtbare Einsätze" style="display: flex; align-items: center;"></span>
                <span id="credit-sum-display" title="Durchschnittliche Credits" style="display: flex; align-items: center;"></span>
                <span id="patient-sum-display" title="Patienten" style="display: flex; align-items: center;"></span>
                <span id="prisoner-sum-display" title="Gefangene" style="display: flex; align-items: center;"></span>
                <span id="endangered-mission-count-display" title="Gefährdete Einsätze (älter als 24h)" style="display: flex; align-items: center;"></span>
            `;
        ...
        `;
        // Füge Stile nur hinzu, wenn der <head> existiert
        if (document.head) {
        ...
        console.error("[LSS Summenleiste mit Warnung] Ein schwerwiegender Fehler ist aufgetreten und wurde abgefangen:", e);
        alert("Ein Fehler im Skript 'Summenleiste mit Warnung' ist aufgetreten. Die Seite sollte trotzdem normal laden. Bitte prüfe die Entwicklerkonsole (F12) für Details.");
    }
})();
