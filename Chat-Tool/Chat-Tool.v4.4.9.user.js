...
// @version      4.4.9
// @description  Kombiniert Messenger-Design (v3.0) mit Blocker & Ladefunktion (v3.5). NEU: Smiley-Auswahl, ASCII-Smiley-Konvertierung, Linkify & Bild-Vorschau/Modal (v4.4.9).
...
(async function() {
        ...
        modal.innerHTML = `
            <div class="pro-chat-modal-content">
                <div class="pro-chat-modal-header">
                    <h3 style="margin: 0;">Chat-Einstellungen (Pro v4.4.9)</h3>
     ...
     * Wird in v4.4.9 erweitert, um Bildlinks zu erkennen.
    ...
    function linkifyText(text) {
        ...
        // Muss auch Bild-Endungen enthalten
        ...
        return text.replace(urlRegex, (url) => {
            ...
            // **NEU (v4.4.9): Wenn es ein Bildlink ist, erstelle eine Vorschau**
    ...
    /**
     * NEUE FUNKTION: Konvertiert getippte ASCII-Smileys in Unicode-Emojis (v4.4.7)
     */
    function convertAsciiToUnicode(text) {
        let convertedText = text;
        // Konvertiere die Schlüssel mit längster Zeichenkette zuerst, um kürzere Matchs zu verhindern.
        const sortedKeys = Object.keys(SMILEY_MAP).sort((a, b) => b.length - a.length);

        for (const ascii of sortedKeys) {
            const regex = new RegExp(
                // Entschärfe alle Regex-speziellen Zeichen im ASCII-Smiley
                ascii.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&'),
                'g'
            );
            // Ersetze nur, wenn der Smiley nicht Teil eines Wortes ist
            convertedText = convertedText.replace(regex, (match, offset, string) => {
                const prevChar = string[offset - 1];
                const nextChar = string[offset + match.length];
                
                // Prüfen, ob vor oder nach dem Smiley ein alphanumerisches Zeichen steht.
                // Nur ersetzen, wenn keine Wortzeichen (a-z, 0-9, _) direkt angrenzen.
                const isWordChar = (char) => char && /\w/.test(char);

                if (!isWordChar(prevChar) && !isWordChar(nextChar)) {
                    // Ersetzen durch das Unicode-Emoji
                    return SMILEY_MAP[match];
                }
                return match; // Nicht ersetzen, wenn es Teil eines Wortes ist
            });
        }
    ...
    function processMainChatMessage(node) {
        ...
        for (const child of Array.from(node.childNodes)) {
            ...
            if (child.nodeType === 3 && usernameSpan && !usernameSpan.contains(child)) {
                ...
                // **NEU: 5a. Linkify (URLs in <a>-Tags/Vorschauen umwandeln)**
    ...
    // === NEUE FUNKTION: Eingabefeld zu Textarea umwandeln (AKTUALISIERT v4.4.7) ===
    function replaceInputWithTextarea() {
        ...
        // proChatTextarea.style.cssText = inputElement.style.cssText; // NICHT übernehmen, da es interne Stile stört
        ...
        // 6. Event-Listener für das automatische Wachsen
        const autoGrow = (el) => {
            el.style.height = 'auto'; // Wichtig: Zurücksetzen, um Verkleinern zu ermöglichen
            el.style.height = (el.scrollHeight) + 'px';
        };

        // 7. Input-Listener (AutoGrow + Whisper-Anzeige)
        ...
        // 8. Keydown-Listener (Senden) - AKTUALISIERT (v4.4.7)
        ...
        // 9. Elemente ZUM DOM HINZUFÜGEN (IN KORREKTER REIHENFOLGE)
        const formElement = inputElement.closest('form');
        if (formElement) {
             // 9a. Füge Whisper-Anzeige VOR dem input-group-addon (Send-Button-Container) hinzu
             formElement.prepend(whisperTargetDisplay);
        }

        // 9b. Ersetze das Input-Feld (innerhalb von .input-group)
        inputElement.parentNode.replaceChild(proChatTextarea, inputElement);

        // 9c. Füge den Smiley-Addon-Container an das Ende der .input-group
        inputGroup.appendChild(smileyInputGroupAddon);

        // 9d. Füge den Popover-Container VOR dem .input-group in den Eltern-Container
        inputGroup.parentNode.insertBefore(smileyContainer, inputGroup);

        // 10. Label zum Papierflieger machen
        ...
        // 11. Initial einmal Größe anpassen
        setTimeout(() => autoGrow(proChatTextarea), 100);
    }

    // === STYLES (KOMBINIERT v4.4.9) ===
    const mergedStyles = `
        /* === EINSTELLUNGS-MODAL === */
        ...
        /* === NEU: SMILEY-CONTAINER POPUP (POSITION FIX) === */
        #pro_chat_smiley_container {
            position: absolute;
            bottom: 250px; /* Reduziert von 60px auf 50px für näher am Feld */
            right: 0;
            width: 270px;
            padding: 10px;
            /* Dark Mode */
        ...
        /* === PAGE-LOADER STYLES (Unverändert) === */
        ...
        /* === NEU: BILD-VORSCHAU UND MODAL STYLES (v4.4.9) === */
        ...
        .pro-chat-image-preview-container {
            ...
            line-height: 0; /* Entfernt unnötigen Raum unter dem Bild */
        ...
        .pro-chat-image-preview {
            ...
            height: auto;
            display: block;
            min-height: 50px; /* Fallback für Ladezustand */
            object-fit: cover;
        }
        ...
        .pro-chat-modal-image {
            display: none;
            position: fixed;
            z-index: 10000; /* Sehr hoch, über allen anderen Modals */
            left: 0;
            top: 0;
            width: 100%;
            height: 100%;
        ...
        #proChatImageModalImage {
            max-width: 100%;
            max-height: 85vh; /* 85% der Viewport-Höhe */
            display: block;
            border-radius: 5px;
            object-fit: contain;
        }
    ...
    // 3. Bild-Modal vorbereiten (wird erst bei Klick sichtbar)
    createImageModal();

    // 4. Kombinierte Einstellungs-UI erstellen (Modal + Header Controls)
    ...
    // 7. Paginierungs-Setup für /alliance_chats
    ...
    // 9. Globalen Klick-Listener für Flüster- & Reply-Icons hinzufügen
    document.addEventListener('click', (e) => {
