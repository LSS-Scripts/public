...
// @version      4.6.6 // Version erhöht und alle Positionierungsfehler behoben + /w Autocomplete
...
(async function() {
    ...
    function createSettingsUI() {
        ...
        // NEU: Chatverlauf-Button auf "Neuer Tab" umbiegen
        // HIERHER VERSCHOBEN und mit document.querySelector
        const chatVerlaufLink = document.querySelector('a[href="/alliance_chats"].lightbox-open');
        if (chatVerlaufLink) {
            chatVerlaufLink.classList.remove('lightbox-open');
            chatVerlaufLink.target = '_blank';
            chatVerlaufLink.rel = 'noopener noreferrer';
        }
        ...
        modal.innerHTML = `
            <div class="pro-chat-modal-content">
                <div class="pro-chat-modal-header">
                    <h3 style="margin: 0;">Chat-Einstellungen</h3>
                ...
                <div class="pro-chat-modal-body">
                    <details open>
                        ...
                        <fieldset>
                            <legend>Geblockte User</legend>
                            <div class="pro-chat-setting-row" style="grid-template-columns: 1fr auto;">
                                <input type="text" id="lss_blocker_name_input" placeholder="User-Namen aus Verband suchen..." list="lss_blocker_userlist">
                                <button id="lss_blocker_name_add_btn">Hinzufügen</button>
                            </div>
                            <datalist id="lss_blocker_userlist"></datalist>
        ...
        // 4b. NEU: Datalist für Blocker-Suche füllen
        const datalist = document.getElementById('lss_blocker_userlist');
        if (datalist && allianceUsers.length > 0) {
            allianceUsers.forEach(user => {
                const option = document.createElement('option');
                option.value = user.name;
                // Optional: ID im Datensatz speichern, falls man sie braucht
        ...
        document.getElementById('lss_blocker_name_add_btn').addEventListener('click', addBlockedUserByName);
        document.getElementById('lss_blocker_name_input').addEventListener('keypress', (e) => { if (e.key === 'Enter') addBlockedUserByName(); });
    ...
    function populateBlockerList() {
        ...
        blockedIdArray.forEach(id => {
            ...
            // NEU: Finde den Namen zur ID
            const user = allianceUsers.find(u => u.id === id);
            const displayName = user ? `${user.name} (ID: ${id})` : `Unbekannte ID: ${id}`;

            li.innerHTML = `<span>${displayName}</span> <button data-id="${id}" class="lss_blocker_id_remove_btn">Löschen</button>`;
     ...
     */
/**
     * Fügt einen User anhand des Namens zur Blocker-Liste hinzu
     */
    function addBlockedUserByName() {
        const inputElement = document.getElementById('lss_blocker_name_input');
        if (!inputElement) return;
        const nameToAdd = inputElement.value.trim();

        // Finde den User im allianceUsers-Array
        const user = allianceUsers.find(u => u.name.toLowerCase() === nameToAdd.toLowerCase());

        if (!user) {
            alert('Benutzer nicht in der Verbandsliste gefunden. Bitte Namen exakt aus der Vorschlagsliste wählen.');
            return;
        }

        const idToAdd = user.id;

        // Ab hier fast identisch zur alten Funktion
        if (blockedIdArray.includes(idToAdd)) {
            alert(`Benutzer '${user.name}' (ID: ${idToAdd}) ist bereits in der Liste.`);
            inputElement.value = ''; // Eingabe leeren
            return;
        }
        if (autoExcludedAdminIds.includes(idToAdd)) {
             alert(`Benutzer '${user.name}' (ID: ${idToAdd}) ist ein Admin und kann nicht geblockt werden.`);
             inputElement.value = ''; return;
        }

        blockedIdArray.push(idToAdd);
        saveBlockedIds();
        populateBlockerList(); // Diese Liste muss auch angepasst werden (Schritt 3e)
...
function parseUsernamesFromHtml(html) {
        ...
        const users = []; // Umbenannt von usernames zu users
        userLinks.forEach(link => {
            const username = link.textContent.trim();
            const profileId = link.href.split('/').pop(); // NEU: ID extrahieren

            if (username.length > 0 && profileId && username !== myUsername) { // Eigenen Namen ausschließen
                users.push({ name: username, id: profileId }); // NEU: Objekt speichern
            }
        });
        return users;
    }
    ...
    async function fetchAllAllianceMembers() {
        ...
        try {
          ...
          // 4. Duplikate (anhand der ID) entfernen und alphabetisch (anhand des Namens) sortieren
            ...
            const uniqueUsernames = Array.from(userMap.values()).sort((a, b) => a.name.localeCompare(b.name));
    ...
    function showSuggestions(filteredUsers) {
        ...
        filteredUsers.slice(0, 10).forEach((user, index) => { // 'username' zu 'user'
            const item = document.createElement('div');
            item.className = 'pro-chat-suggestion-item';
            item.textContent = user.name; // NEU: user.name
            item.dataset.index = index;
            item.addEventListener('click', () => selectSuggestion(user.name)); // NEU: user.name
