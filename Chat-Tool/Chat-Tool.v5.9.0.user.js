// ==UserScript==
// @name         LSS - Moderner Chat Pro & Status (v5.9.0 - Layout & Sticky Whisper)
// @namespace    http://tampermonkey.net/
// @version      5.9.0
// @description  Chat v4.6.6 Design + Blocker (klarer) + Sticky Favoriten + Header Fix.
...
(async function() {
    ...
    let activeWhisperUser = null; // NEU: Speichert den aktuell aktiven Dauer-Flüster-Partner
    let allianceUsers = [];
    let suggestionContainer = null;

    const SMILEY_MAP = {':)':'😊',':-)':'😊',':(':'🙁',':-(':'🙁',';)':'😉',';-)':'😉',':D':'😃',':-D':'😃','XD':'🤣','x)':'🤣','xD':'😂',':o':'😮',':-o':'😮',':*':'😘',':-*':'😘','o.O':'🤔','O.o':'🤔',':p':'🤪',':P':'🤪',':-p':'🤪',':-P':'🤪','8)':'😎','8-)':'😎','<3':'❤️','O:)':'😇','O:-)':'😇','^^':'😀',':/':'😒',':-/':'😒','T_T':'😭',';(':'😢',':\'(':'😢',':|':'😐',':-|':'😐','>:(':'😡','>:-(':'😡','B)':'🤓','B-)':'🤓','P:':'💩'};
    const UNICODE_SMILIES = ['😃','😂','🤣','😉','😊','🤔','😮','🙁','😡','😢','😭','😈','💩','😍','😘','😎','🤩','🥳','🥶','🥵','😴','🤪','🤭','🤫','🤥','😬','🤒','🤕','🤯','😵','💀','🤷','🤤','🚑','🚒','🚓','🚁','🚚','🚛','🚨','👨‍🚒','👩‍🚒','👮','👮‍♀️','👮‍♂️','🧑‍⚕️','👷','🧑‍🔧','⛑️','🦺','🗣️','🤷‍♀️','🏡','🏢','🏥','🏦','🏫','🔥','💨','💥','💣','⚡','💧','🌊','🚧','⚠️','☢️','☣️','💉','💊','🩹','🩸','🩺','😷','🥽','🧤','🪓','🔪','🛢️','🛏️','🦼','📄','💬','🍕','🍔','🥐','🌮','🍪','☕','🍺','🍾','🍎','🍇','🍉','🍌','🐕','🐈','🌳','🌲','☀️','🌙','🌟','🌈','🌧️','☔','❄️','💨','🌬️','🛠️','⚙️','🔗','🔒','🔑','🔨','🔦','📻','📞','🔋','💻','📱','⌚','📚','💰','💵','🎁','🎈','💍','⚽','🎤','🎼','🎲','✈️','🚂','⏎','⇧','☝️','👍','👎','🙏','👋','👏','✊','✋','🛑','🚫','✅','❌','❕','❓','❗','⬆️','⬇️','👈','👉','👆','👇','🖕','⏱️','⏳','📅','🗺️','⭐','🎉','❤️','💯'];
    ...
    function renderFavoritesBar() {
        ...
        favoritesIdArray.forEach(id => {
            ...
            if (activeWhisperUser === u.name) {
                btn.classList.add('active'); // Style wenn aktiv
            }
    ...
    function createSettingsUI() {
        ...
        if(head) {
            // Layout Fix: "Chat" left, Controls right
            head.style.display = 'flex';
            head.style.justifyContent = 'space-between'; // Not strictly needed with margin-left:auto but good practice
            ...
            d.style.marginLeft = 'auto'; // This pushes the container to the far right!
            d.style.gap = '8px';

            d.innerHTML = `
                <div style="display:flex; align-items:center; margin-right:5px;">
                    <button id="proChatSettingsBtn" class="btn btn-default btn-xs" style="margin-right:5px;"><span class="glyphicon glyphicon-cog"></span></button>
                    <span style="font-size:0.85em; font-weight:bold; margin-right:5px; opacity:0.8;">Blocker:</span>
                    <label class="lss-switch lss-switch-xs">
                        <input type="checkbox" id="lss_blocker_header_toggle">
                        <span class="lss-slider"></span>
                    </label>
                    <span id="lss_blocker_header_count" class="lss-blocker-count"></span>
                </div>
            `;
            ...
            // Append instead of prepend, but CSS moves it visually
            head.appendChild(d);
        }
    ...
    function replaceInputWithTextarea() {
        ...
        proChatTextarea.onkeydown = function(e) {
            if(e.key === 'Enter' && !e.shiftKey) {
                ...
                // NEU: Auto-Whisper Logic (Dauerhaft flüstern)
                if (activeWhisperUser && val.length > 0 && !val.toLowerCase().startsWith('/w ')) {
                    this.value = `/w ${activeWhisperUser} ${val}`;
                    val = this.value.trim(); // Wert für den Safety Check aktualisieren
                }
    ...
    GM_addStyle(`
        ...
        #safetySlider { -webkit-appearance: none; width: 100%; height: 100%; background: transparent; position: absolute; top: 0; left: 0; margin: 0; z-index: 2; cursor: grab; }
        #safetySlider:focus { outline: none; }
        #safetySlider::-webkit-slider-thumb { -webkit-appearance: none; appearance: none; width: 50px; height: 50px; border-radius: 50%; background: radial-gradient(circle at 30% 30%, #f0ad4e, #d35400); border: 4px solid #fff; box-shadow: 0 4px 10px rgba(0,0,0,0.5); margin-left: 5px; transition: transform 0.1s; }
        #safetySlider::-webkit-slider-thumb:active { transform: scale(1.1); cursor: grabbing; }
        .nippel-success .nippel-lasche-zone { background: rgba(40, 167, 69, 0.8) !important; }
        .nippel-success .nippel-lasche-zone::after { content: "OK!"; color: white; transform: rotate(0); }
    `);
