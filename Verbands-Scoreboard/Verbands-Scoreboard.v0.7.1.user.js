...
(function() {
    ...
    let currentMissions = []; // Cache für die Live-Einsätze zur Detail-Anzeige
    ...
    function extractMissions(responseText, sourceName) {
        ...
        try {
            ...
            console.error(`JSON Parse Error in '${sourceName}':`, e);
            console.error("Fehlerhafter String:", jsonStr);
            throw new Error(`Konnte die Daten von '${sourceName}' nicht verarbeiten.`);
        }
    }

    // DATENVERARBEITUNG
    function analyzeAndMergeMissions(ownMissions, allianceMissions, existingStats, processedMissionIds) {
        const stats = JSON.parse(JSON.stringify(existingStats));
        const processMission = (mission) => {
            if (!mission.id || processedMissionIds.has(mission.id)) return;
        ...
        [...ownMissions, ...allianceMissions].forEach(mission => {
            if (mission.alliance_shared_at) processMission(mission);
        });
