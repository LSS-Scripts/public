// ==UserScript==
// @name         LSS Kilometerkönig "Fun Fact Edition"
// @namespace    www.leitstellenspiel.de
// @version      1.5.2
// @description  Statistik-Dashboard. Fix für Buttons/Header und erweiterte "Sinnlos-Fakten".
// @author       Gemini
...
(function() {
    ...
    const SPACE_DISTANCES = [
        { km: 149600000, name: "Distanz zur Sonne", icon: "☀️" },
        { km: 54600000, name: "Distanz zum Mars (min)", icon: "🔴" },
        { km: 384400, name: "Distanz zum Mond", icon: "🌑" },
        { km: 40075, name: "Erdumrundungen", icon: "🌍" }
    ];

    const EARTH_MILESTONES = [
        { km: 0, text: "Nicht mal aus der Garage gefahren." },
        { km: 50, text: "Kurz Brötchen holen." },
        { km: 100, text: "Einmal zur nächsten Großstadt." },
        { km: 500, text: "Einmal quer durch die Niederlande." },
        { km: 1000, text: "Einmal quer durch Deutschland (Nord-Süd)." },
        { km: 2448, text: "Einmal Route 66 (USA)." },
        { km: 3500, text: "Einmal Tour de France Länge." },
        { km: 6400, text: "Volle Länge des Amazonas." },
        { km: 9288, text: "Transsibirische Eisenbahn (Moskau-Wladiwostok)." },
        { km: 12742, text: "Einmal quer durch den Erdkern." },
        { km: 21196, text: "Die Chinesische Mauer entlang." },
        { km: 30000, text: "Fast einmal um die Welt." }
    ];
    ...
    const vehicleTypes = {
        ...
        '107': 'LF-L', '108': 'AB-L', '109': 'MzGW SB', '110': 'NEA50', '112': 'NEA200', '111': 'NEA50 (FW)', '113': 'NEA200 (FW)',
    ...
    const css = `
        ...
        .modal-title { color: #ffffff !important; text-shadow: none !important; font-weight: bold; }
        ...
        /* BUTTONS (Schließen & Filter) - Wichtig: Hintergrundbilder/Gradienten entfernen */
        .modal-footer .btn-default, .km-sidebar-header .btn-default {
        ...
        .modal-footer .btn-default:hover, .km-sidebar-header .btn-default:hover {
            background-color: #555 !important;
            border-color: #777 !important;
        }
        .close { color: #fff !important; opacity: 0.8; text-shadow: none !important;}
        ...
        .km-total-fun { font-size: 13px; color: #aaa; margin-left: 10px; font-style: italic; }
    `;
                ...
                <div class="modal-dialog km-modal-dialog" role="document">
                    <div class="modal-content km-modal-content">
                        <div class="modal-header">
                            ...
                            <h4 class="modal-title">📊 Fuhrpark-Statistik</h4>
                        </div>
    ...
    function getFunFact(km) {
        // 1. Check Space
        for (let space of SPACE_DISTANCES) {
            if (km >= space.km) {
                const count = (km / space.km).toFixed(2);
                return `${space.icon} ${count}x ${space.name}`;
            }
        }
        // 2. Check Earth Milestones (Rückwärts loopen, um das höchste passende zu finden)
        for (let i = EARTH_MILESTONES.length - 1; i >= 0; i--) {
            if (km >= EARTH_MILESTONES[i].km) {
                return `📍 ${EARTH_MILESTONES[i].text}`;
            }
        }
    ...
    function renderTables(activeTypeIds) {
        ...
        const funFact = getFunFact(totalKm);
