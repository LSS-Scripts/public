// ==UserScript==
// @name         Benötigte Fahrzeuge WIRKLICH verschönern - MIT ZEITCHECK UND FAHRZEUGMARKIERUNG
// @namespace    http://tampermonkey.net/
// @version      18.3.12 
// @description  Stabile und schnelle Anzeige der benötigten Fahrzeuge mit allen Features, Zeitcheck und Markierung zu langsamer Fahrzeuge in der Fahrzeugliste.
// @author       B&M & KI (Ergänzung für Zeitprüfung & Markierung)
...
(function() {
    ...
    GM_addStyle(`
        ...
        /* NEU: "Zeitüberschritten"-Zustand mit rotem Leuchten */
        #missing-vehicles-container-custom.time-exceeded {
            background: linear-gradient(145deg, #442a2a, #683535);
            border-color: #d9534f;
        ...
        /* NEU: Stil für ZU LANGSAME ANFORDERUNGEN (oben) */
        #missing-vehicles-container-custom .missing-vehicles-table tr.vehicle-row-too-slow td {
            background-color: rgba(217, 83, 79, 0.3);
            color: #ffb3b3;
            font-weight: bold;
        }
        
        /* NEU: Stil für die tatsächlichen zu langsamen Fahrzeuge (unten in der Alarmliste) */
        #mission_vehicle_driving tbody tr.vehicle-too-slow, 
        #vehicle_select tbody tr.vehicle-too-slow {
            background-color: rgba(217, 83, 79, 0.4) !important; 
            border-left: 5px solid #d9534f;
            animation: pulse-red 1s infinite alternate;
        }

        /* Animation für besonders wichtige Warnung */
        @keyframes pulse-red {
            from { box-shadow: 0 0 5px rgba(217, 83, 79, 0.6); }
            to { box-shadow: 0 0 10px rgba(217, 83, 79, 1); }
        }
        ...
        #missing-vehicles-container-custom .progress-bar-text {
            ...
            color: #fff; /* Wichtig: Eindeutige Farbe */
            font-weight: bold;
            font-size: 0.9em;
            text-shadow: 1px 1px 2px rgba(0, 0, 0, 0.7); /* Wichtig: Starker Schatten */
            text-align: center;
        }
    ...
    function getMissionCountdownSeconds() {
        const countdownSpan = document.querySelector('span[id^="mission_countdown_"]');
        if (countdownSpan) {
            const timeText = countdownSpan.textContent.trim();
            const parts = timeText.split(':');
            
            let timeStringNormalized = timeText;
            if (parts.length === 2) {
                timeStringNormalized = '00:' + timeText;
            } else if (parts.length === 1) {
                 timeStringNormalized = '00:00:' + timeText;
            }
            
            return parseTime(timeStringNormalized);
        }
        return null;
    }

    /**
     * Sammelt ALLE Anfahrtszeiten (in Sekunden) pro alarmiertem Fahrzeugtyp.
     * @returns {Object<string, number[]>} Map von vehicleTypeId zu Array von Anfahrtszeiten in Sekunden (aufsteigend sortiert).
     */
    function getAllDrivingVehicleTimes() {
        const driveTimes = {};
        
        // Helferfunktion, um Anfahrtszeiten zu sammeln
        const collectTime = (vehicleTypeId, timeInSeconds) => {
            if (vehicleTypeId && timeInSeconds > 0) {
                driveTimes[vehicleTypeId] = driveTimes[vehicleTypeId] || [];
                driveTimes[vehicleTypeId].push(timeInSeconds);
            }
        };

        // 1. Rollende Fahrzeuge
        document.querySelectorAll('#mission_vehicle_driving tbody tr').forEach(row => {
            ...
            const driveTimeCell = row.querySelector('td[id^="vehicle_drive_"]');
            
            if (vehicleTypeId && driveTimeCell) {
                const driveTimeInSeconds = parseInt(driveTimeCell.getAttribute('sortvalue'), 10);
                collectTime(vehicleTypeId, driveTimeInSeconds);
            }
        });

        // 2. Ausgewählte (aber noch nicht rollende) Fahrzeuge
        document.querySelectorAll('.vehicle_checkbox:checked').forEach(checkbox => {
             const row = checkbox.closest('tr');
            if (!row) return;

            const driveTimeCell = row.querySelector('td[id^="vehicle_arrival_"]');
            const driveTimeInSeconds = driveTimeCell ? parseInt(driveTimeCell.getAttribute('sortvalue'), 10) : 0;
        ...
        // Zeiten für jeden Typ sortieren (kleinste Zeit zuerst)
        for (const type in driveTimes) {
            driveTimes[type].sort((a, b) => a - b);
        }
    ...
    /**
     * NEUE FUNKTION: Markiert einzelne Fahrzeuge in der unteren Liste, deren Anfahrt zu lange dauert.
     */
    function markDrivingVehicles() {
        if (missionCountdownSeconds === null || missionCountdownSeconds <= 0) {
            // Wenn kein Countdown vorhanden ist, alle Markierungen entfernen und abbrechen.
            document.querySelectorAll('.vehicle-too-slow').forEach(el => el.classList.remove('vehicle-too-slow'));
            return;
        }
        
        // Liste aller Fahrzeugzeilen (rollend ODER ausgewählt)
        const allVehicleRows = Array.from(document.querySelectorAll('#mission_vehicle_driving tbody tr, #vehicle_select tbody tr'));

        // Alle vorhandenen Markierungen entfernen
        allVehicleRows.forEach(el => el.classList.remove('vehicle-too-slow'));
        ...
        allVehicleRows.forEach(row => {
            ...
            if (row.closest('#mission_vehicle_driving')) {
                timeCell = row.querySelector('td[id^="vehicle_drive_"]');
            } 
            // 2. Ausgewählte Fahrzeuge
            else if (row.closest('#vehicle_select')) {
                timeCell = row.querySelector('td[id^="vehicle_arrival_"]');
            }

            if (timeCell) {
                driveTimeInSeconds = parseInt(timeCell.getAttribute('sortvalue'), 10);
            }
            
            // Prüfen, ob die Anfahrtszeit den Countdown überschreitet
            if (driveTimeInSeconds > 0 && driveTimeInSeconds > missionCountdownSeconds) {
                row.classList.add('vehicle-too-slow');
            }
        });
    }

    // --- Originale Konstanten und Mappings ---
    ...
    const REQUIREMENT_MAP = {
        ...
        'betreuungs--und-verpflegungsausstattung':['130','132'],'betreuungs--und-verpflegungsausstattungen':['130','132'],'polizeipferde':['134','135','136'],'polizeipferd':['134','135','136'],
    ...
    "Polizisten": {
        ...
        "FüKW (Polizei)": 3,
        "GefKw": 2,
        "Polizeihubschrauber": 3,
        "Polizeimotorrad": 1,
        "FuStW (DGL)": 2,
        "Zivilstreifenwagen": 2,
        "WaWe 10": 5,
        "SEK - ZF": 4,
        "SEK - MTF": 9,
        "MEK - ZF": 4,
        "MEK - MTF": 9,
        "DHuFüKW": 2,
        "Pferdetransporter klein": 4,
        "Pferdetransporter groß": 2,
        "Zugfahrzeug Pferdetransport": 6,
        "Polizeihubschrauber mit verbauter Winde": 3,
        "LauKw": 5
    },
    "Polizist": {
        "FuStW": 2,
    ...
    "Rettungsdienstler": {
        ...
        "GW-San": 6,
        "ELW 1 (SEG)": 2,
        "Rettungshundefahrzeug": 5,
        "GW UAS": 4,
        "GW-Bt": 3,
        "Bt-Kombi": 9,
        "Bt LKW": 3,
        "GW-Bergrettung (NEF)": 6,
        "GW-Bergrettung": 6,
        "ELW Bergrettung": 3,
        "ATV": 1,
        "Hundestaffel (Bergrettung)": 5,
        "Schneefahrzeug": 1,
        "RTH Winde": 2,
        "GW-Höhenrettung (Bergrettung)": 4,
        "GW TeSi": 5,
        "LKW Technik (Notstrom)": 6,
        "MTW TeSi": 7
    },
    "Personen mit GW-Gefahrgut-Ausbildung": {
        "GW-Gefahrgut": 3
    },
    "Person mit GW-Gefahrgut-Ausbildung": {
        "GW-Gefahrgut": 3
    },
    ...
    // Globale Variablen für den Zeitcheck-Status
    let missionCountdownSeconds = null;
    let allDriveTimesByType = {};
    ...
    function initialize() {
        ...
        if (vehicleReqDiv) {
            ...
            while ((match = regex.exec(requirementText)) !== null) {
                ...
                requirementsState[reqName] = { needed: parseInt(match[1], 10), fulfilled: 0, vehicleTypeIds: vehicleTypeIds, tooSlow: false }; 
            }
        }
    ...
    function updateCountsAndRedraw() {
        ...
        missionCountdownSeconds = getMissionCountdownSeconds();
        allDriveTimesByType = getAllDrivingVehicleTimes();
        isTimeExceeded = false;

        // 1. Vehicle Pool erstellen 
        ...
        // 2. Fahrzeug-Erfüllung und Zeitprüfung berechnen
        for (const reqName in requirementsState) {
            const requirement = requirementsState[reqName];
            let fulfilledCount = 0;
            requirement.tooSlow = false;
            
            // Erfüllte Anzahl zählen
            ...
            // Zeitprüfung:
            if (missionCountdownSeconds !== null && requirement.needed > 0) {
                
                let allRelevantDriveTimes = [];
                requirement.vehicleTypeIds.forEach(vehicleId => {
                    const times = allDriveTimesByType[vehicleId] || [];
                    allRelevantDriveTimes = allRelevantDriveTimes.concat(times);
                });
                
                if (allRelevantDriveTimes.length === 0) continue; 
                
                allRelevantDriveTimes.sort((a, b) => a - b);
                
                // Kritischer Index: Nötiges Fahrzeug - 1
                const criticalIndex = requirement.needed - 1;
                
                if (criticalIndex >= 0 && allRelevantDriveTimes.length > criticalIndex) {
                    const criticalTime = allRelevantDriveTimes[criticalIndex];
                    
                    if (criticalTime > missionCountdownSeconds) {
                        requirement.tooSlow = true;
                        isTimeExceeded = true;
                    }
                }
            }
        ...
        // 3. Personal-Erfüllung berechnen (unverändert)
        const personnelPool = {};
        ...
        // NEU: Individuelle Fahrzeug-Markierung auf der unteren Liste
        markDrivingVehicles(); 
    ...
    function renderTable() {
        ...
        const isMissionFullyOK = allRequirementsMet && !isTimeExceeded;

        // Titel basierend auf dem Status festlegen
        let vehicleTitle = 'Benötigte Fahrzeuge';

        if (isTimeExceeded) {
            const countdownTime = missionCountdownSeconds !== null ? new Date(missionCountdownSeconds * 1000).toISOString().substr(11, 8) : 'Unbekannt';
            vehicleTitle = `🚨 ZEITÜBERSCHREITUNG! (Countdown: ${countdownTime}) - Letzte Fahrzeuge brauchen zu lange!`;
        ...
        // Helfer-Funktion zur Generierung der Tabellenzeilen
        const generateBody = (entries) => {
            let bodyHtml = '';
            for (const [name, counts] of entries) {
                const { needed, fulfilled, tooSlow } = counts;
                ...
                let rowClass = isMet ? 'vehicle-row-ok' : '';
                
                // Markiere die Zeile rot, wenn die Nötige erfüllung zu langsam ist.
                if (tooSlow) {
                    rowClass = 'vehicle-row-too-slow'; 
                }

                // HIER ist der wichtige Teil für die Anzeige des Klartexts
                bodyHtml += `<tr class="${rowClass} ${departmentClass}"><td>${name}</td><td><div class="progress-bar-container"><div class="progress-bar" style="width: ${progress}%; background-position: ${backgroundPosition}% 0;"></div><div class="progress-bar-text">${fulfilled} / ${needed}</div></div></td></tr>`;
            }
            return bodyHtml;
        };

        // --- HTML Zusammenstellung (unverändert) ---
        ...
        customContainer.classList.toggle('all-vehicles-ok', isMissionFullyOK);
        customContainer.classList.toggle('time-exceeded', isTimeExceeded);
    }
