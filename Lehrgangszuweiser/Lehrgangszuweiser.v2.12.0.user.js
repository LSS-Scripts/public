// ==UserScript==
// @name        Lehrgangszuweiser - Erweitert (Deep Count Fix)
// @version     2.12.0
// @license     BSD-3-Clause
// @author      B&M & BOS-Ernie (Hybrid by Gemini)
// @description Fügt Buttons hinzu, um Personen einer Wache auf einmal einem Lehrgang zuzuweisen. Version 2.12.0: Korrekte "Soll-Ist"-Berechnung auch für Lehrgänge mit Voraussetzungen (z.B. Windenoperator), indem das vorhandene Personal tiefengeprüft wird.
...
(function () {
  ...
  // *** REPORTING POPUP ***
  function showReport(missingLog) {
      $("#lss-assigner-report-modal").remove();
      if (missingLog.length === 0) return;

      let htmlRows = "";
      missingLog.forEach(entry => {
          htmlRows += `
              <tr>
                  <td>${entry.name} <a href="/buildings/${entry.id}" target="_blank">🔗</a></td>
                  <td style="text-align:center">${entry.needed}</td>
                  <td style="text-align:center">${entry.assigned}</td>
                  <td style="text-align:center; font-weight:bold; color:#e74c3c;">-${entry.missing}</td>
              </tr>
          `;
      });

      const modalHtml = `
      <div id="lss-assigner-report-modal" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 9999; display: flex; justify-content: center; align-items: center;">
          <div style="background: #fff; padding: 20px; border-radius: 8px; width: 90%; max-width: 600px; max-height: 80vh; display: flex; flex-direction: column; box-shadow: 0 5px 15px rgba(0,0,0,0.5); color: #333;">
              <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #ddd; padding-bottom: 10px; margin-bottom: 10px;">
                  <h4 style="margin:0; color: #e74c3c;">⚠️ Fehlendes Personal (${missingLog.length} Wachen)</h4>
                  <div>
                      <button id="lss-report-copy-btn" class="btn btn-default btn-xs">Kopieren</button>
                      <button onclick="$('#lss-assigner-report-modal').remove()" class="btn btn-primary btn-xs">Schließen</button>
                  </div>
              </div>
              <div style="overflow-y: auto; padding-right: 5px;">
                  <p style="margin-top: 0;">Bei folgenden Wachen wurde das Ziel nicht erreicht:</p>
                  <table class="table table-striped table-condensed text-left" style="width:100%; font-size: 13px; margin-bottom: 0;">
                      <thead>
                          <tr>
                              <th>Wache</th>
                              <th style="text-align:center">Soll</th>
                              <th style="text-align:center">Ist</th>
                              <th style="text-align:center">Fehlt</th>
                          </tr>
                      </thead>
      ...
      if ($("body").hasClass("dark")) {
          $("#lss-assigner-report-modal > div").css({ "background": "#2c3a4a", "color": "#e1e8ed" });
          $("#lss-assigner-report-modal table").css("color", "#e1e8ed");
          $("#lss-assigner-report-modal table thead th").css("border-bottom", "1px solid #555");
          $("#lss-assigner-report-modal > div > div:first-child").css("border-bottom", "1px solid #555");
      }

      $("#lss-report-copy-btn").on("click", function() {
          const text = missingLog.map(e => `${e.name}: Soll ${e.needed} / Ist ${e.assigned} (Fehlt: ${e.missing})`).join("\n");
          navigator.clipboard.writeText(text).then(() => {
              $(this).text("Kopiert!");
              setTimeout(() => $(this).text("Kopieren"), 2000);
          });
      });
  }
  ...
  function extractRelevantExtensions(buildingMap) {
      ...
      visibleBuildingElements.forEach(element => {
          ...
          if (buildingData && buildingData.extensions) {
              buildingData.extensions.forEach(ext => uniqueExtensions.add(ext.caption));
          }
      });
  ...
  async function autoAssignPersonnel(event) {
    ...
    // Alten Report entfernen
    $("#lss-assigner-report-modal").remove();
    const missingPersonnelLog = [];
    ...
    for (let i = 0; i < totalPanels; i++) {
        ...
        if (requiredEducationKey === 'none') {
            ...
            $(`.schooling_checkbox[building_id='${buildingId}']`).each(function() {
                 if ($(this).attr(targetEducationKey) === 'true') {
                     existingCount++;
                 }
            });
            console.log(`[LSS-Debug] Wache ${buildingId}: Soll ${userInputNumber}, Ist ${existingCount} (Deep Check)`);

            if (ignoreExisting) {
                neededForThisBuilding = userInputNumber;
            } else {
                neededForThisBuilding = Math.max(0, userInputNumber - existingCount);
            }
        }

        if (neededForThisBuilding > 0) {
            const maxGlobalPossible = Math.min(neededForThisBuilding, schoolingFree);
            
            await selectPersonnel(buildingId, maxGlobalPossible, requiredEducationKey, targetEducationKey);
            
            // Wie viele wurden TATSÄCHLICH in diesem Durchlauf neu markiert?
            const actuallyAssignedThisTurn = countSelectedPersonnel(buildingId) - alreadySelected;
            totalAssignedInRun += actuallyAssignedThisTurn;
            
            // Wenn wir weniger markiert haben, als wir eigentlich bräuchten...
            if (actuallyAssignedThisTurn < neededForThisBuilding) {
                missingPersonnelLog.push({
                    id: buildingId,
                    name: buildingSearchName,
                    needed: neededForThisBuilding,
                    assigned: actuallyAssignedThisTurn,
                    missing: neededForThisBuilding - actuallyAssignedThisTurn
                });
            }
        }
    }

    if (missingPersonnelLog.length > 0) {
        showReport(missingPersonnelLog);
        updateStatus(`FERTIG! ${totalAssignedInRun} gewählt (siehe Popup)`, "label-danger");
    } else {
