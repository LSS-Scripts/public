// ==UserScript==
// @name         Massenkategorierer
// @namespace    http://tampermonkey.net/
// @version      19.0.0.
// @description  Die endgültige Version, die die korrekte und unlimitierte API (/api/vehicles) nutzt.
// @author       Masklin
...
(function() {
    ...
    // Feste Liste zur Übersetzung von ID zu Name
    ...
    async function populateVehicleTypes() {
        ...
        try {
            // Wir nutzen die neue, bessere API und Standard-fetch
            const vehiclesResponse = await fetch('https://www.leitstellenspiel.de/api/vehicles');
            if (!vehiclesResponse.ok) throw new Error(`API-Antwort: ${vehiclesResponse.status}`);

            // Diese API liefert direkt das Array, kein .result-Objekt nötig
            const myVehicles = await vehiclesResponse.json();

            if (!Array.isArray(myVehicles)) {
                throw new Error("API-Antwort ist keine Liste (Array).");
            }

            const enrichedVehicles = myVehicles.map(vehicle => ({
                ...vehicle,
                vehicle_type_name: vehicleNameMapping[vehicle.vehicle_type] || `Unbekannter Typ (${vehicle.vehicle_type})`
            }));
            window.allMyVehicles = enrichedVehicles;

            const ownedTypeNames = [...new Set(enrichedVehicles.map(v => v.vehicle_type_name))].sort();
    ...
    async function runProcess(isDelete = false) {
        ...
        for (let i = 0; i < total; i++) {
            ...
            try {
                // Hier verwenden wir wieder fetch für die Update-Anfrage, da diese einzeln sind.
    ...
    function init() {
        createModal();
        const graphicPacksLi = document.getElementById('graphic_packs');
        if (graphicPacksLi) {
            ...
            newLink.href = '#';
            newLink.innerHTML = '<span class="glyphicon glyphicon-pencil"></span> Massen-Editor';
            newLink.onclick = (e) => {
                e.preventDefault();
